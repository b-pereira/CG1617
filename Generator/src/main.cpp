...
std::string triangle_to_string(Point3d pointA, Point3d pointB, Point3d pointC, Point3d normalA, Point3d normalB, Point3d normalC)
{
    return point3D_to_string(pointA) + " " + point3D_to_string(normalA)  + "\n" +
           point3D_to_string(pointB) + " " + point3D_to_string(normalB)  + "\n" +
           point3D_to_string(pointC) + " " + point3D_to_string(normalC);
}

std::string triangle_to_string(Point2d textPointA, Point2d textPointB, Point2d textPointC)
{
    return  point2D_to_string(textPointA)  +" " +
            point2D_to_string(textPointB)  +" " +
...
vector<float> cross(vector<float> a, vector<float> b)
{
    ...
    vector<float> res = {a[1] * b[2] - a[2] * b[1],
                         a[2] * b[0] - a[0] * b[2],
                         a[0] * b[1] - a[1] * b[0]};
    return res;
}
...
vector<float> getPartialDerivativeBezierPatchPointParamU(float u, float v, MatrixP P)
{
...
vector<float> getPartialDerivativeBezierPatchPointParamV(float u, float v,  MatrixP P)
{
...
Point3d getNormalBezierPatchPoint(float u, float v,  MatrixP P)
{
    ...
    vector<float> normal_tmp = cross(derivU, derivV);
            ...
            for (i = 0; i < tesselation; ++i)
            {
                for (j = 0; j < tesselation; ++j)
                {
                    ...
                    Point3d normalB = getNormalBezierPatchPoint(i / tesselation,
                                      (j + 1) / tesselation, mat);
                    Point3d normalC = getNormalBezierPatchPoint((i + 1) / tesselation,
                                      (j + 1) / tesselation, mat);
                    Point3d normalD = getNormalBezierPatchPoint((i + 1) / tesselation,
                                      j / tesselation, mat);

                    Point2d texCoordsA(i / tesselation,
                                       j / tesselation);
                    Point2d texCoordsB(i / tesselation,
                                       (j + 1) / tesselation);
                    Point2d texCoordsC((i + 1) / tesselation,
                                       (j + 1) / tesselation);
                    Point2d texCoordsD ((i + 1) / tesselation,
                                        j / tesselation);
                    ...
                    << " ";
                    ficheiro << triangle_to_string(normalA, normalB, normalD)
                    << " ";
                    ficheiro << triangle_to_string(texCoordsA, texCoordsB, texCoordsD)
                    << endl;


                    ficheiro << triangle_to_string(pointB, pointC, pointD)
                    << " ";
                    ficheiro << triangle_to_string(normalB, normalC, normalD)
                    << " ";
                    ficheiro << triangle_to_string(texCoordsB, texCoordsC, texCoordsD)
                    << endl;
